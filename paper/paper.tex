...
{\centering \includegraphics[width=1\linewidth]{paper_files/figure-latex/Skill-rel2bench-1} 

}

\caption{Pinball skill score for the seven best performing approaches.}\label{fig:Skill-rel2bench}
\end{figure}

Figure \ref{fig:Skill-rel2bench} illustrates the trade-off between calibration and pinball skill.
...
\caption{RMSE values over the forecast horizon.}\label{fig:lead-time-rmse}
\end{figure}
...
\caption{Speed (Running time) vs. forecast accuracy}\label{fig:time-accuracy}
\end{figure}

Finally, Figure \ref{fig:time-accuracy} reports the forecast accuracy of each approach presented in Table \ref{tab:tab-results} versus the computational time required to generate the forecast for a given forecasting horizon of 48 hours. The x-axis represents the speed of each method as slow, moderate or fast.
