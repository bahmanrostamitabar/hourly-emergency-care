---
title:  Forecasting hourly hospital admission
author:
  - name: Author1
...
2. We develop a novel approach to forecast day-ahead hourly hospital admission using Generalised Additive Models for Location Scale and Shape, which accounts for i) the bounded and non-Gaussian distribution of admissions, ii) multiple seasonalities, weather and holiday effects, and  iii) variation in forecast uncertainty;
...
knitr::kable(li, booktabs = T, linesep = "",caption = "Summary of studies in hourly emergency care forecasting") %>% kableExtra::kable_styling(latex_options = c("scale_down","hold_position"), font_size =11) %>% column_spec(3,"6em") %>%
  column_spec(4,"4em") %>% column_spec(6,"15em")  %>% column_spec(7,"4em") 
```
...
We develop a range of models for forecasting hospital admissions using a multi-year dataset from a hospital in the UK. In order to facilitate model development and testing, we divide the data into a training and test period. We use cross-validation to iteratively fit, evaluate and develop models using the training data, before settling on their optimal configuration and testing them on the as-yet unseen test data. Data from 2014-04-01 to 2018-02-28 are used for training, and from 2018-03-01 to 2019-02-28 for testing.


Forecast are issued every day at midnight and mid-day for lead-times from 0 to 48 hours-ahead on a rolling basis. In order to compare both parametric and non-parametric probabilistic forecasts, we evaluate quantiles of the predictive distribution at probability level 0.05, 0.10,... 0.95, as well as the predictive expectation. 

In the remainder of this section we explore the data in more detail, describe the forecast evaluation metrics we will employ, and introduce a range of benchmark models for producing forecasts. The benchmarks serve as reference methods and include some simple methods as well as more sophisticated routines designed to work `out-of-the-box', such as Facebook's Prophet. The performance of these methods provides an important reference, and any proposed novel solution should offer material benefits to justify increased complexity.

## Data exploration {#data}

The data we use in this case study are from hospital XYZ, and comprises records of arrivals at Accident and Emergency at hourly granularity. The training data runs from 01/04/2014 to 28/02/2018, and the test data runs from 1/3/2018 to 28/2/2019.
...
The first benchmark we consider is to simply use the empirical distribution of past observations as a probabilistic forecast for same hour-of-day and day-of-week that we are forecasting. This is often called `climatology'. However, as we observe a trend in the data used here, we re-calculate the climatology on a rolling basis using the most recent one-year of data in order to track the trend.
...
Note that we have also tried the Normal distribution in our experiments, but as expected it was not suitable and performed poorly. This is because it is symmetric around the mean, while the data exhibits skewness.
...
(\#eq:poissonreg)
\end{equation}
<!-- where we initially consider an additive model for $\lambda_t$ of the form -->
<!-- \begin{equation} -->
<!--   \log(\lambda_t) = \sum_{i=1}^7 \beta_i D_i(t) + \sum_{j=1}^7 D_j(t) f_j(H(t)) + t f_\text{Y}(Y(t)) + f_\text{Temp}(Y(t),C_t) \quad , -->
<!--  (\#eq:additivemodel) -->
<!-- \end{equation} -->
<!-- where the functions $H(t)$ and $Y(t)$ return the hour of the day (1--24) and day of the year (1-366) at time $t$, respectively, $D_i(t)$ is a dummy variable for day of the week at $t$, and $C_t$ is the temperature at time $t$. Analysis of this model fit on the training data revealed systematic biases which were identified as relating to public and school holidays. We therefore expand the model to include these effects as follows, -->
<!-- \begin{equation} -->
<!--   \log(\lambda_t) = \sum_{i=1}^10 \beta_i D^{+}_i(t) + \sum_{j=1}^10 D^{+}_j(t) f_j(H(t)) + \sum_{k=1}^2 \beta_{10+k} S_k(t) + \sum_{l=1}^2 S_l(t) f_l(H(t)) + t + f_\text{Y}(Y(t)) + f_\text{Temp}(Y(t),C_t) \quad , -->
<!--  (\#eq:additivemodel_v2) -->
<!-- \end{equation} -->
<!-- where $D^{+}_i(t)$ is a dummy variable for day type including Monday to Sunday, Public Holiday, Christmas Day and New Year's Day, and $S_k(t)$ is a dummy variable for School Holidays. -->
where we consider an additive model for $\lambda_t$ of the form
\begin{equation}
\log(\lambda_t) = \sum_{i=1}^10 \beta_i D^{+}_i(t) + \sum_{j=1}^10 D^{+}_j(t) f_j(H(t)) + \sum_{k=1}^2 \beta_{10+k} S_k(t) + \sum_{l=1}^2 S_l(t) f_l(H(t)) + t + f_\text{Y}(Y(t)) + f_\text{Temp}(Y(t),C_t) \quad ,
(\#eq:additivemodel_v2)
\end{equation}
where the functions $H(t)$ and $Y(t)$ return the hour of the day (1--24) and day of the year (1-366) at time $t$, respectively, where $D^{+}_i(t)$ is a dummy variable for day type including Monday to Sunday, Public Holiday, Christmas Day and New Year's Day, $S_k(t)$ is a dummy variable for School Holidays, and $C_t$ is the temperature at time $t$. Public and school holidays were included following experiments on the training data which revealed systematic biases associated with these periods.
...
\begin{align*}
\mu_t = &  \sum_{i=1}^10 \beta_i D^{+}_i(t) + \sum_{j=1}^10 D^{+}_j(t) f_j(H(t)) + \sum_{k=1}^2 \beta_{10+k} S_k(t) + \sum_{l=1}^2 S_l(t) f_l(H(t)) + t + f_\text{Y}(Y(t)) + f_\text{Temp}(Y(t),C_t) \quad, \\
...
The objective of density forecasts it to be as sharp as possible while remaining reliable/calibrated [@Gneiting2007a]. A forecast is said to be sharp if the predictive distribution has a relatively small spread, indicating low uncertainty, which is valuable to decision makers provided the forecast is calibrated.
...
### Truncated Normal GAMLSS Model

We can support the interpretation the models given in Equation \@ref(eq:additivemodel_NOtr_v2) after the parameters have been estimated by visualising their reposes to inputs. Figure \@ref(fig-NOtr-model) shows the effect of time of day and day-type on the level and spread of predicted arrivals.

```{r fig-NOtr-model}

load("NOtr2_model.Rda")

plot_data <- data.frame(Hour = 0:23)
for(i in c("Mon","Tue","Wed","Thu","Fri","Sat","Sun","Public Holiday","Christmas Day","New Years Day")){
  pef <- gamlss::getPEF(h2_gamlss2,term="clock_hour",parameter = "mu",fixed.at = list(dow3=i))
  plot_data[[i]] <- pef(0:23)
}

plot_data <- reshape::melt.data.frame(plot_data,id.vars="Hour",variable_name="Day Type")

plot_1 <- ggplot(plot_data,aes(x=Hour,y=value,color=`Day Type`)) + geom_smooth(se = F,span=0.5) +
  ylab("Effect [Expected Arrivals]") + theme_few()

# plot_1

plot_data2 <- data.frame(Hour = 0:23)
for(i in c("Mon","Tue","Wed","Thu","Fri","Sat","Sun","Public Holiday","Christmas Day","New Years Day")){
  pef <- gamlss::getPEF(h2_gamlss2,term="clock_hour",parameter = "sigma",fixed.at = list(dow3=i))
  plot_data2[[i]] <- pef(0:23)
}

plot_data2 <- reshape::melt.data.frame(plot_data2,id.vars="Hour",variable_name="Day Type")

plot_2 <- ggplot(plot_data2,aes(x=Hour,y=value,color=`Day Type`)) + geom_smooth(se = F,span=0.5) +
  ylab("Effect [Standard Deviation of Arrivals]") + theme_few()
...
Probabilistic forecasts are evaluated following the principle of *sharpness subject to calibration*, meaning that the sharper forecast is prefered provided that it is calibrated. Mis-calibrated forecasts are unsuitable for use in decision-making so should be excluded. Calibration is evaluated visually in \@ref(Reliability), which highlights a systematic negative bias accross all probability levels in many models, with only the truncated normal GAMLSS models (NOtr-2) and ADAM-iETS models showing good calibration across all probability levels. Notably, benchmarks exhibit negative quantile bias as they struggle to capture the long term trend of increasing attendance.


```{r Reliability, fig.cap= "Reliability diagram for selected methods. If forecasts are calibrated, the Empirical frequency that each probability level is deceded should match the Nominal frequency, and the reliability diagram should be a straight line on the diagonal (black dashed line).", fig.align='center',fig.width= 7,fig.height=6}
```


Evaluation metrics from all Methods on the test period are presented in \@ref(tab-results) which are ordered by Quantile Bias. The five models identified above have a Quantile Bias of 0.014 or less, which is substantially lower than the next group of forecast with Quantile Biases of 0.037 and above.



```{r tab-results, out.width= "50%"}
results_table <- read_rds("results_table.rds")
results_table <- results_table[!results_table$Method%in%c("ADAM-iETSX-Ceiling","LinearRegression","Poisson-2-I","fasster","qreg-1"),]
results_table <- results_table[order(results_table$`Quantile Bias`),]
...
The Pinball Score evaluates overall performance, penalising both poor calibration and lack of sharpness, and ranks the various models quite differently. Pinball is visualised in Figure \'ref(Pinball) broken down by quantile. NBI-2 and Poisson-2 achieve the lowest Pinball Loss, but are poorly calibrated so not suitable for use in decision making. Of the well calibrated methods, NOtr-2 and Ttr-2 have the lowest Pinball Score, and provide a significant improvement over Benchmark 2 by this metric. However, we note that while the improvement in terms of Pinball Score appears small, these methods have substantially improved forecast calibration compared to the benchmarks, which are poorly calibrated.


```{r Pinball, fig.cap= "Pinball loss for selected forecsating methods and quantiles from 5% to 95%. Several methods perform similarly well relative to others by this metric but calibration should also be considered when evaluating probabilistic foreasts.", fig.align='center',fig.width= 7,fig.height=6}
```

In order to verify that this improvement is a result of improved modelling and not a result of sample variation, we have performed block bootstrap resampling on Pinball skill scores relative to Bechmark-2 for each forecast. The results for all methods with a skill score greater than -2% are shown in Figure \@ref(Skill_rel2bench_reduced), where boxes are coloured according to calibration. Where boxes do not overlap we can be confident that apparent differences in skill are not the result of sample variation.

```{r Skill_rel2bench_reduced, fig.cap= "Bootstrapped skill scores for top-performing methods relative to Benchmark-2. Non-overlapping boxes indicate that we can be confident that apparent differences in Pinball Score are not the result of sample variation.", fig.align='center',fig.width= 7,fig.height=6}
```


```{r Pinaball_vs_Qbias, fig.cap= "Pinball Score vs average Qunatile Bias for selected methods. While some methods perform well by one measure or the other, only NOtr-2 is both calibrated and has a low Pinball Score.", fig.align='center',fig.width= 7,fig.height=6}
```


We delve deeper into forecast performance by evaluating forecasts by time of day, and lead-time. The Pinball Score for slected methods is plotted against lead-time in Figure \@ref(lead-time-pb), separated by whether the forecast was issued at midnight (00) or mid-day (12). We can see clearly that time of day is a greater influence on performance than lead-time, with morning arrivals and the evening peak in arrivals proving the most difficult to predict.

```{r lead-time-pb, fig.cap= "Pinball Score by lead-time for forecasts issued at midnight (00) and mid-day (12).", fig.align='center',fig.width= 7,fig.height=6}
```


Computational time should also be considered, as national public health bodies may be required to produce forecasts of many variables for a large number of hospitals on  regular basis...



# Conclusion {#conclusion}

1. Conditional distribution of future arrivals is neither Gaussian nor Poisson distributed, and requires careful treatment.

2. Simple benchmarks and "out-of-the-box" forecasting methods produce poor predictions of A&E arrivals.

3. A distributional regression appraoch, with additive models for both the location and scale parameters of a truncated Normal distirbution produces best all-round forecast performance
