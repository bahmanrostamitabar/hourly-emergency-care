...
plot(Bench_mqr[issueTime==issue,grep("q",colnames(h2_mqr)),with=F],
     xlab="Lead-time [hours]",ylab="Attendance",main=paste0("Origin: ",issue," (",format(issue,"%A"),")"),
     ylim=c(0,40),Legend = "topleft")

reliability(Bench_mqr[,grep("q",colnames(h2_mqr)),with=F],h2$n_attendance,kfolds = h2$kfold)
pinball(Bench_mqr[,grep("q",colnames(h2_mqr)),with=F],h2$n_attendance,kfolds = h2$kfold)
...
JB_results$qreg_boost_V1$expectation <- mean_from_qs(mqr = JB_results$qreg_boost_V1)
class(JB_results$qreg_boost_V1) <- c("MultiQR",class(JB_results$qreg_boost_V1))
