...
electronic medical records and other [@ROSTAMITABAR20221197].
...
```{r seasonplot-dofw, fig.cap= "Subseries plot: day of weekly arrivals", fig.align='center',out.width="70%"}
```
...
power outages, and schools closed in the UK [@stormemma2018].
...
\textcolor{blue}{
The dataset used in this study does not include the period of the COVID-19 pandemic. During COVID-19, the dynamic of ED has changed. That means any forecasting model that is used during such periods to forecast ED arrivals needs to be modified to reflect those changes for that specific period. However, we expect that if the COVID period is taken into account by the models (or interpolated), the models should perform similarly.}
