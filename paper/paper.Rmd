...
We develop a range of models for forecasting hospital admissions using a multi-year dataset. In order to facilitate model development and testing, we divide the data into a training period. We use cross-validation to iteratively fit, evaluate and develop models using the training data, before settling on their optimal configuration and testing them on the as-yet unseen test data.

Forecast are produced/issue at midnight and mid-day for lead-times from 0 to 48 hours-ahead on a rolling basis. In order to compare both parametric and non-parametric probabilistic forecasts, we evaluate quantiles of the predictive distribution at probability level 0.05, 0.10,... 0.95, as well as the expectation. 


## data {#data}

The data we use in this case study are from hospital XYZ, and comprises records of arrivals at Accident and Emergentcy at hourly granularity. The training data runs from 01/04/2014 to 28/02/2018, and the test data runs from 1/3/2018 to 28/2/2019.
...
**Not sure if this should stay here, but interesting and worth reporting, I think...**

Plot smooth functions estiamted for time of day, day of year, for both mean and variance parameters.
